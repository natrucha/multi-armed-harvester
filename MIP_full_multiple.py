...
def analysisMultiple(n_runs, run_list, time_list, print_out):
    '''Obtain average and variance of results and time-to-run for all the runs'''

    time_array = np.zeros(n_runs)
    FPE_array  = np.zeros(n_runs)
    FPT_array  = np.zeros(n_runs)

    for i_run in range(n_runs):
        time_array[i_run] = time_list[i_run].total_seconds()
        ...
        FPE_array[i_run]  = run_list[i_run][0].FPE * 100
        FPT_array[i_run]  = run_list[i_run][0].FPT

    avg_time = np.average(time_array)
    var_time = np.var(time_array)
    print('time array (since the var is insane):', time_array)
    print('average time for every run, {:.3f}'.format(avg_time), '+/-{:.3f} s'.format(var_time)) 
    print()  

    avg_FPE = np.average(FPE_array)
    var_FPE = np.var(FPE_array)
    print('average FPE for every run, {:.3f}'.format(avg_FPE), '+/-{:.3f}%'.format(var_FPE)) 
    print() 

    avg_FPT = np.average(FPT_array)
    var_FPT = np.var(FPT_array)
    print('average FPT for every run, {:.3f}'.format(avg_FPT), '+/-{:.3f} fruits/s'.format(var_FPT))  
    print() 
...
def main():
    ...
    for i_run in range(n_runs):
        ...
        ###### ****** somehow, running MIP_melon will cause y_lim to chnage to whatever was calculated in MIP_melon. FIX THIS
    ...
    analysisMultiple(n_runs, run_list, time_list, print_out)
